...
class FrontendServerTest(unittest.TestCase):
    ...
    def test_oidc_frontend_does_not_expose_session_credentials(self) -> None:
        ...
        self.assertIn(b"BroadcastChannel", shared_script)
        self.assertIn(b"auth_popup", shared_script)
        self.assertIn(b"window.location.reload()", shared_script)
        self.assertIn(b"window.close()", shared_script)
