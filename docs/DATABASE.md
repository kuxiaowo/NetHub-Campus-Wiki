...
- 前台打开 Yearbook 阅读器会让 `hot` 加 1；热度使用通用节流逻辑，同一登录账户对同一对象 5 秒内只会增加一次。前台点击普通资源链接、Yearbook PDF 或 Yearbook 单页图片下载会让 `downloads` 加 1，下载数不节流。后台预览不计入热度和下载数。
- Yearbook 资源不新增字段，复用 `resource_url` 作为目录 URL。该目录必须位于 `public/` 下，目录内放所有页面图片和 PDF 文件；页面图片支持 `.jpg`、`.jpeg`、`.png`、`.webp`、`.gif`。封面不单独维护，接口会自动使用文件名自然升序的第一张图片作为 `image` 展示值。前台阅读器按文件名自然升序展示图片，并使用文件名自然升序的第一个 `.pdf` 作为下载文件。建议命名为 `001.png`、`002.png`、`yearbook.pdf`。
...
- `downloads` 是活动级统计，前台下载活动整包压缩文件或下载单张放大照片都会增加该字段。
...
当前前端保留“下载压缩文件”入口，同时照片放大弹窗也提供单张下载按钮。点击整包下载或单张照片下载都会增加 `photo_activities.downloads`。
