...
const photoYear = document.querySelector('#photoYear');
const photoSort = document.querySelector('#photoSort');
...
function resourceCard(resource) {
  ...
  const icon = resourceIconMap[resource.category] || resourceIconMap.other;
...
async function loadResourceMeta() {
  ...
  photoYear.innerHTML = '<option value="">全部年份</option>' +
    meta.photoYears.map((year) => `<option value="${escapeHtml(year)}">${escapeHtml(year)}</option>`).join('');
...
function renderActivityList(activities) {
  ...
  activityList.querySelectorAll('[data-activity-id]').forEach((button) => {
    button.addEventListener('click', () => {
      selectedActivityId = Number(button.dataset.activityId);
...
async function loadPhotoActivities() {
  ...
  const params = new URLSearchParams();
  if (photoYear.value) params.set('year', photoYear.value);
  if (photoSearch.value.trim()) params.set('search', photoSearch.value.trim());
  params.set('sort', photoSort.value);
...
function downloadCurrentActivity() {
  activePhotoItems.forEach((item, index) => {
    ...
    link.href = safeExternalUrl(item.src);
...
[resourceYear, resourceSort].forEach((control) => control.addEventListener('change', loadResources));
resourceSearch.addEventListener('input', () => {
  clearTimeout(resourceTimer);
  resourceTimer = setTimeout(loadResources, 300);
});

[photoYear, photoSort].forEach((control) => control.addEventListener('change', loadPhotoActivities));
photoSearch.addEventListener('input', () => {
  clearTimeout(photoTimer);
  photoTimer = setTimeout(loadPhotoActivities, 300);
});

downloadActivity.addEventListener('click', downloadCurrentActivity);
...
loadResourceMeta()
  .then(() => Promise.all([loadResources(), loadPhotoActivities()]))
  .catch((error) => {
